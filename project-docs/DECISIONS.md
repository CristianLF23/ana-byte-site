...
## 24/09: redesign desktop contínuo

O pedido posterior substitui a navegação por botões apenas no desktop. A home larga reúne obras, processo, Ana e início de projeto em fluxo vertical nativo; o mobile conserva as cenas e ações aprovadas. A cidade ocupa uma abertura cinematográfica. Depois, superfícies editoriais de leitura e fotografias de trabalhos reais substituem a repetição de quatro panoramas. O acervo visível usa a lista canônica de 19 obras já presente no HTML; o formulário inline reutiliza a mensagem de WhatsApp existente, sem guardar dados. GSAP e ScrollTrigger locais coordenam revelações e deslocamentos sutis, desligados em `prefers-reduced-motion`; CSS e HTML garantem conteúdo legível quando a animação falha. Não foi necessário instalar Three.js, Lenis, React, componentes externos ou dependências pagas. O estudo anterior da ERA e a observação atual justificam a cadência, não autorizam cópia de código, assets, layout ou geometria.
